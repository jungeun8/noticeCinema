			...
			<div class="d-flex justify-content-between">
				...
				<div class="col-7 ps-5" style="border-left: 1px #e9e9e9 solid;">
					...
					<div class="pt-2 pb-3" id="name">
						<p class="fs-3"><strong>[CINEMABAX]</strong><br>${snackInfo.name}</p>
						<p class="bundle">${snackInfo.detail}</p>
					</div>
					<div class="pb-4">
						<strong>
							<span id="price" style="color: black; font-size: 20px" class="fs-3">
								<fmt:formatNumber type="number" maxFractionDigits="0" value="${snackInfo.price}" /></span> 원
						</strong>
					</div>
							...
							<table class="table">
								...
								<tbody>
									<tr class="align-middle">
										...
										<td colspan="2"><span style="color:red; font-weight:bold">※ 사용가능극장은 당사의 사정에 의해 변경될 수 있습니다.</span></td>
									</tr>
									<tr class="align-middle">	
										<th>유효기간</th>
										<td colspan="2"><span>구매일로부터 90일 이내 사용 가능합니다.</span><br>
									</tr>
									<tr class="align-middle">	
										<th>판매수량</th>
									...
									<tr class="align-middle">	
										<th>구매 후 취소</th>
										<td colspan="2"><span>구매일로부터 10일 이내 취소 가능합니다.</span></td>
									</tr>	
									<tr class="align-middle">
										<th>구매수량</th>
										<td>
										...
										<td>
											<input type="button" class="btn btn-outline fs-3" id="itemSum" value="<fmt:formatNumber type="number" maxFractionDigits="0" value="${snackInfo.price}" />" readonly> <strong>원</strong>
										</td>
									</tr>
								</tbody>
							</table>
...
$(function() {
	...
	$("#buy #insert-purchase").click(function() {
		...
		$.ajax({
			type: "post",
			url: "purchase",	// 구매기능
			data: {snackNo:$("#snackNo").val(), orderAmount:parseInt($("#number").text()), userId:'${LOGINED_USER.id}'},
